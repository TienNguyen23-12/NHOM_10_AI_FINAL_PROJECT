...
class DispatchCenter:
    ...
    def evaluate_and_dispatch(self, acc_pos, grid_map):
        """Hàm tính toán nhanh (Dùng khi tắt Visualizer)"""
        best_hospital = None
        best_cost = float('inf')
        best_path = None
    ...
    def evaluate_generator(self, acc_pos, grid_map):
        """Hàm Visualizer: Quét sóng Radar Vàng/Cam nhẩm tính từng bệnh viện"""
        best_hospital = None
        best_cost = float('inf')
        best_path = None
        ...
        for h_name, h_info in self.hospitals.items():
            ...
            while open_set:
                ...
                if current == acc_pos:
                    ...
                    # Yield nhịp cuối báo hiệu đã tính xong trạm này
                ...
                steps += 1
                # Quét 3 ô vẽ 1 lần để nhịp độ êm ái hơn
                if steps % 3 == 0:
                    yield open_set_tracker, visited, path, h_name, False

        # Hoàn tất việc đấu thầu
